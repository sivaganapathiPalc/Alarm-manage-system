      ...
      <table className="w-full">
        <thead className="bg-gray-50">
          <tr>
            ...
            {hasAccess(isAdmin?.role, PERMISSIONS.ACKNOWLEDGE) && (
              <th className="px-4 py-3 text-left">
                <input
                  type="checkbox"
                  onChange={(e) => onSelectAll(e.target.checked)}
        ...
        <tbody className="divide-y">
          {loading ? (
            <tr>
              <td colSpan={isAdmin.role === 'admin' ? 7 : 6} className="px-4 py-8 text-center">
                Loading...
              </td>
            </tr>
          ) : alarms.length === 0 ? (
            <tr>
              <td
                colSpan={isAdmin.role === 'admin' ? 7 : 6}
                className="px-4 py-8 text-center text-gray-500"
              >
                No alarms
              </td>
...
function AlarmsTab(props) {
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-1 md:flex-row md:items-center">
        {hasAccess(props.userProfile?.role, PERMISSIONS.FILTER_ALARMS) && (
        <Filters
          filters={props.filters}
          onFilterChange={props.onFilterChange}
          onResetFilters={props.onResetFilters}
        ...
        <div className="md:ml-4 md:flex-1">
          ...
          {hasAccess(props.userProfile?.role, PERMISSIONS.SAVE_FILTERS) && (
          <SavedViews
            savedViews={props.savedViews}
            onApplyView={props.onApplyView}
            onDeleteView={props.onDeleteView}
